...
class FonNeyman:
    ...
    def _running_mode(self, ptr):
        curr_ptr = ptr
        while self._mem[curr_ptr] != 255:
            if self._mem[curr_ptr] == 1:
                self._mov(self._mem[curr_ptr+1], self._mem[curr_ptr+3])
            elif self._mem[curr_ptr] == 2:
                self._add(self._mem[curr_ptr+1], self._mem[curr_ptr+3])
            elif self._mem[curr_ptr] == 3:
                self._inp(self._mem[curr_ptr+3])
            elif self._mem[curr_ptr] == 4:
                self._out(self._mem[curr_ptr+3])
            elif self._mem[curr_ptr] == 255:
                break
            elif self._mem[curr_ptr] == 5:
                self._mem[1] = self._mem[self._mem[curr_ptr+3] + 3]
            elif self._mem[curr_ptr] == 6:
                self._mem[2] = self._mem[curr_ptr + 3]
            elif self._mem[curr_ptr] == 7:
                self._mem[1] -= 1
                if self._mem[1] > 0:
                    curr_ptr = self._mem[2]
            curr_ptr += 4
        ...
        while True:
            ...
            if com[0] == "help":
                print("You can use following commands to manage the machine:\n"
                      ...
                      "\"run [prog_name] [args]\" - run custom program")
            elif com[0] == "shutdown":
                break
            elif com[0] == "prog":
